...
// read
// https://msdn.microsoft.com/en-us/library/windows/desktop/aa366542(v=vs.85).aspxf
...
* @return
**/
pmap_context open_map_context(_In_ const wchar_t* file_path, IN bool ReadOnly, OUT PFILE_CTX& Ctx, IN PLARGE_INTEGER pOffset, DWORD dwoffsetSize)
{
    _ASSERTE(NULL != file_path);
    if (NULL == file_path) return false;
    ...
    )
{
	...
	memset(&Offset, 0x1, sizeof(Offset));
	
    // map src, dst file
	pmap_context src_ctx = open_map_context(src_file, true, ctx, Offset, sizeof(Offset));
